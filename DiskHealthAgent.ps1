# ==============================================================================
#  DiskHealth Agent v5.1 - Windows Agent
...
function Get-AgentId {
    ...
    if ($machineGuid) {
        try {
            ...
            # Set UUID version 5 nibble (SHA-1 namespace)
            $hash[6] = ($hash[6] -band 0x0F) -bor 0x50
            # Set UUID variant bits (RFC 4122 variant 1)
...
function ConvertTo-SafeJson {
    ...
    if ($obj -is [string]) {
        $s = $obj
        # Escape backslash first, then double-quote, then the standard escape sequences
        $s = $s -replace '\\','\\'
        $s = $s -replace '"','\"'
        $s = $s -replace "`r",'\r'
        $s = $s -replace "`n",'\n'
        $s = $s -replace "`t",'\t'
        # Escape remaining ASCII control characters (0x00-0x1F, 0x7F) as \uXXXX
        $out = New-Object System.Text.StringBuilder
        foreach ($c in $s.ToCharArray()) {
            $cp = [int][char]$c
            if ($cp -lt 0x20 -or $cp -eq 0x7F) {
                $out.Append([string]::Format('\u{0:x4}', $cp)) | Out-Null
            } else {
...
function Update-Self {
    ...
    try {
        ...
        # Update tray script if server has one
        try {
        ...
        # Relaunch updated agent and exit this instance
        # NOTE: exit 0 terminates this process; no code after it is reachable.
...
function Get-SmartctlData {
    ...
    try {
        ...
        foreach ($line in $scanLines) {
            ...
            if ($line -match '^(/dev/\S+)\s+-d\s+(\S+)') {
                $devices += @{ path=$matches[1]; dtype=$matches[2] }
            } elseif ($line -match '^(/dev/\S+)') {
                $devices += @{ path=$matches[1]; dtype='auto' }
            }
        }
        foreach ($dev in $devices) {
            try {
                ...
                if ($nv) {
                    ...
                    if ($null -ne $e.available_spare -and $e.available_spare -le 10) { $e.predict_failure = $true }
                }
                if ($d.ata_smart_attributes -and $d.ata_smart_attributes.table) {
                    ...
                    if ($null -ne $e.reallocated -and $e.reallocated -gt 0) { $e.predict_failure = $true }
                }
...
function Get-WmiSmartData {
    ...
    try {
        ...
        foreach ($s in $statuses) {
            ...
            $e = @{
                predict_failure=[bool]$s.PredictFailure
                temperature=$null; reallocated=$null; pending=$null; uncorrectable=$null
                smartctl_used=$false
            }
            if ($rawLookup.ContainsKey($inst)) {
                $raw = $rawLookup[$inst]
                # Each attribute entry is 12 bytes; first 2 bytes are a header, so start at offset 2.
                # Guard: need at least 14 bytes (header + 1 attribute) to iterate safely.
                if ($raw -and $raw.Count -ge 14) {
                    for ($i = 2; $i -le ($raw.Count - 12); $i += 12) {
                        $id = $raw[$i]
                        if ($id -eq 0) { continue }
...
function Get-DiskHealth {
    ...
    if ($wmiMap.ContainsKey('_error')) {
        ...
        if ($scMap.Count -eq 0) { Log-Warn "No SMART data (WMI: $wmiErr). Run as Administrator for full data." }
    ...
    foreach ($pd in (Get-WmiObject Win32_DiskDrive | Sort-Object Index)) {
        ...
        if ($s) {
            ...
            if ($s.predict_failure) { $smartStat = 'Critical' }
                ...
                $smartStat = 'Healthy'
                if ($null -ne $realloc   -and $realloc   -gt 0)  { $smartStat = 'Warning'  }
                if ($null -ne $mediaErr  -and $mediaErr  -gt 0)  { $smartStat = 'Warning'  }
                if ($null -ne $pctUsed   -and $pctUsed   -ge 90) { $smartStat = 'Warning'  }
                if ($null -ne $availSpare -and $availSpare -le 10){ $smartStat = 'Critical' }
            }
        ...
        try {
            ...
            foreach ($part in $parts) {
                ...
                foreach ($ld in $logs) {
                    ...
                    if ($tgb -and $tgb -gt 0 -and $null -ne $fgb) { $pct = [math]::Round((($tgb-$fgb)/$tgb)*100,1) }
                    $volumes += @{
                        drive      = $ld.DeviceID
...
function Build-Report {
    param([string]$AgentId, [string]$CmdId = "")
...
function Send-Report {
    ...
    if ($json.Length -lt 10) { Log-Error "JSON too short"; return }
...
function Poll-Commands {
    ...
    if (-not $resp) { return }
    try { $obj = $resp | ConvertFrom-Json; $commands = $obj.commands } catch { return }
    # Use @() to ensure .Count works correctly in PS 2.0 where a single object lacks .Count
...
function Main {
    ...
    while ($true) {
        ...
        try {
            ...
            if (($pollCount % 10) -eq 0) { Send-Report -AgentId $agentId }
