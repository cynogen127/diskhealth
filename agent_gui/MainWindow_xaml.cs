...
namespace SnipeAgent
{
    ...
    public partial class MainWindow : Window
    {
        ...
        public MainWindow()
        {
            ...
            // parse command-line switches for unattended operations
        ...
        {
            // async void: must catch all exceptions or they crash the app silently
            try
            {
            var args = Environment.GetCommandLineArgs();
            if (args.Length <= 1) return;

            // hide UI while running unattended task (window may briefly appear)
            this.Visibility = Visibility.Hidden;
            AppendAgentLog("Running in command‑line mode...");

            if (args[1].Equals("-uninstall", System.StringComparison.OrdinalIgnoreCase))
            {
            ...
            else if (args[1].Equals("-install", System.StringComparison.OrdinalIgnoreCase) ||
                     args[1].Equals("-update", System.StringComparison.OrdinalIgnoreCase))
            {
                ...
                AppendAgentLog($"Starting {(args[1].Equals("-update", StringComparison.OrdinalIgnoreCase) ? "update" : "install")} to {serverUrl} interval {pollInterval}s...");
                bool ok = await AgentInstaller.RunInstallAsync(serverUrl, pollInterval, msg => AppendAgentLog(msg));
                Application.Current.Shutdown(ok ? 0 : 1);
            }
            else
            {
                AppendAgentLog("Unknown command-line argument: " + args[1]);
                Application.Current.Shutdown(1);
            }
            }
            catch (Exception ex)
            {
                Log.Write($"EXCEPTION in ProcessCommandLineArgs: {ex}");
        ...
        {
            if (int.TryParse(TxtPollInterval.Text.Trim(), out int parsed) && parsed >= 5)
                return parsed;
            return 30;
        }
